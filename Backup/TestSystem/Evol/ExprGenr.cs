    ...
    public class ExprGenr : ExprEvolver
    {
        ...
        public ExprGenr(extree refxp, double start, double end, double prec, double term, int steps, int gen)
        {
            ...
            for (int i = 0; i < 80; i++)
            {
                if (i == 0)
                { 
                    extree x = new extree(expr_type.plus, double.NaN);
                    x.left = new extree(expr_type.powsqrt2, double.NaN);
                    x.right = new extree(expr_type.constant, 0.4);
                    gen_exps[0].Add(x);
                }
                else if (i % 2 == 0)
                {
                    gen_exps[0].Add(GetXFunction(rnm));
                }
                else
                {
                    gen_exps[0].Add(GetConstantFunction(rnm));
                }
            }
        }
        ...
        public override extree ProcessNSteps(int n, bool msg)
        {
            ...
            {
                throw new Exception("Wow, best score " + best_current_score + " ");
            }
        ...
        public override void Step()
        {
            ...
            if (gen_exps[0][0].true_score < best_current_score)
            {
                if (msg_alert)
                {
                    ...
                    Console.Out.WriteLine("With Error Score: " + gen_exps[0][0].true_score);
                }
